<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="视频文件" prop="shipinwenjian">
          <file-upload
          tip="点击上传视频文件"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.shipinwenjian?ruleForm.shipinwenjian:''"
          @change="shipinwenjianUploadChange"
          ></file-upload>
        </el-form-item> 
        <div v-else>
          <el-form-item v-if="ruleForm.shipinwenjian" label="视频文件" prop="shipinwenjian">
            <el-button type="text" size="small" @click="download(ruleForm.shipinwenjian)">预览</el-button>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.ziyuantupian" label="资源图片" prop="ziyuantupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.ziyuantupian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.ziyuanjianjie = true;
          ...
          if(o=='shipinwenjian'){
            this.ruleForm.shipinwenjian = obj[o];
	    this.ro.shipinwenjian = true;
            continue;
          }
          if(o=='shangchuanriqi'){
	...
	this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../ssmkoam8/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yinpinziyuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yinpinziyuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    shipinwenjianUploadChange(fileUrls) {
	this.ruleForm.shipinwenjian = fileUrls;
			this.addEditUploadStyleChange()
    },
